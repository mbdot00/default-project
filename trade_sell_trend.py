...
def main():
    ...
    outcome = "STOPPED"
    log("Running (trend-follow SELL: M5 EMA7<EMA21 and M1 candle red, every 5s, stop if equity <= %.2f or >= %.2f from start)." % (
    ...
    while True:
        ...
        if candle_time != last_candle_time:
            ...
            log(f"M1 {candle_dt} open={open_price:.2f} live={live_price:.2f} "
                f"candle={color} {ema_str} -> {'follow SELL' if is_red and sell_ok else 'hold'}")
            last_candle_logged = candle_time

        if is_red and sell_ok:
            si = mt5.symbol_info(SYMBOL)
            if si is None:
            ...
            result = mt5.order_send(request)
            if result is not None and result.retcode == mt5.TRADE_RETCODE_DONE:
                log(f"SELL opened. ticket={result.order}, price={result.price}")
            else:
                log(f"SELL failed. retcode={result.retcode if result else None} "
                    f"msg={mt5.last_error()}")
        elif color == "green":
            log(f"candle is {color} -> no SELL (green, trend up)")
