...
Parabolic SAR flip strategy for JP225Cash (MT5) using M1 candles.

Strategy:
  - PSAR step=0.01, max_step=0.05 on M1 candles
  - Buy when PSAR is below price (bullish)
  - Sell when PSAR is above price (bearish)
  - Only 1 trade at a time, close before flip
  - Trades IMMEDIATELY when PSAR flips (no :57 wait)
...
def get_psar_direction():
    ...
    # Most recent completed candle (index -2 since -1 is forming)
    recent_psar = df['psar'].iloc[-2]
    recent_close = df['close'].iloc[-2]

    # PSAR below price = bullish (BUY), PSAR above price = bearish (SELL)
...
def log_snapshot():
    ...
    log(f"SNAPSHOT equity={equity:.2f} P/L={pnl:+.2f} longs={longs} shorts={shorts} total={len(magic_positions)}")
...
def main():
    ...
    log(f"Symbol {SYMBOL}, lot {LOT}. PSAR step={SAR_STEP}, max={SAR_MAX} on M1.")
    log(f"Bullish (PSAR<price) -> BUY. Bearish (PSAR>price) -> SELL.")
    ...
    try:
        while True:
            ...
            if direction != last_direction:
                ...
                # No change — just log occasionally
                if now_dt.second == 0:
                    log(f"HOLD {'BUY' if holding == mt5.POSITION_TYPE_BUY else 'SELL' if holding == mt5.POSITION_TYPE_SELL else 'NONE'}. PSAR {direction}.")
