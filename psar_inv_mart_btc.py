"""
psar_inv_mart_btc.py

Inverted PSAR + Martingale strategy for BTCUSD (MT5) using M1 candles.
...
Strategy:
  ...
  - Martingale: lot +0.01 per loss, reset to 0.01 on win
  - No max lot cap
...
SYMBOL = "BTCUSD"
BASE_LOT = float(os.environ.get("MT_LOT", "0.01"))
MAGIC = 920903
...
def close_all_positions():
    ...
    if total_pnl < 0:
        current_lot += 0.01
        log(f"MARTINGALE: loss, lot increased to {current_lot:.2f}")
    else:
        current_lot = BASE_LOT
        log(f"MARTINGALE: win, lot reset to {current_lot:.2f}")
...
def open_trade(order_type, price, lot):
    ...
    result = mt5.order_send(request)
    if result is not None and result.retcode == mt5.TRADE_RETCODE_DONE:
        log(f"{side} opened. ticket={result.order}, price={result.price:.2f}, lot={lot:.2f}")
        return True
    else:
...
def log_snapshot():
    ...
    log(f"SNAPSHOT equity={equity:.2f} P/L={pnl:+.2f} longs={longs} shorts={shorts} total={len(magic_positions)} lot={current_lot:.2f}")
...
def main():
    ...
    log(f"Lot: +0.01 per loss, reset to {BASE_LOT} on win. No max cap.")
            ...
            if direction != last_direction:
                if direction == "bearish":
                    if holding != mt5.POSITION_TYPE_BUY:
                        log(f"PSAR BEARISH (inverted): close all, open BUY lot={current_lot:.2f}")
                        close_all_positions()
                    ...
                    if holding != mt5.POSITION_TYPE_SELL:
                        log(f"PSAR BULLISH (inverted): close all, open SELL lot={current_lot:.2f}")
                        close_all_positions()
                ...
                if now_dt.second == 0:
                    log(f"HOLD {'BUY' if holding == mt5.POSITION_TYPE_BUY else 'SELL' if holding == mt5.POSITION_TYPE_SELL else 'NONE'}. PSAR {direction}. lot={current_lot:.2f}")
